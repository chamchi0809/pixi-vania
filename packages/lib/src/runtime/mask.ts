...
function readPixels(texture: Texture | undefined): Atlas | null {
	...
	try {
		return { data: ctx.getImageData(0, 0, canvas.width, canvas.height).data, w: canvas.width };
	} catch {
...
export function tileMaskFromTextures(
	...
	return (tilesetUid, tileId) => {
		...
		if (!atlases.has(tilesetUid)) atlases.set(tilesetUid, readPixels(textures.get(tilesetUid)));
